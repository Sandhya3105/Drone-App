...
function deepClone(source) {
  if (React9.isValidElement(source) || (0, import_react_is.isValidElementType)(source) || !isPlainObject(source)) {
    return source;
  }
  const output = {};
...
function deepmerge(target, source, options = {
  ...
  if (isPlainObject(target) && isPlainObject(source)) {
    Object.keys(source).forEach((key) => {
      if (React9.isValidElement(source[key]) || (0, import_react_is.isValidElementType)(source[key])) {
...
function useMediaQueryOld(query, defaultMatches, matchMedia, ssrMatchMedia, noSsr) {
  const [match2, setMatch] = React14.useState(() => {
    if (noSsr && matchMedia) {
...
function useMediaQueryNew(query, defaultMatches, matchMedia, ssrMatchMedia, noSsr) {
  const getDefaultSnapshot = React14.useCallback(() => defaultMatches, [defaultMatches]);
  const getServerSnapshot = React14.useMemo(() => {
    if (noSsr && matchMedia) {
      return () => matchMedia(query).matches;
    }
  ...
  const [getSnapshot, subscribe] = React14.useMemo(() => {
    if (matchMedia === null) {
      return [getDefaultSnapshot, () => () => {
      }];
    }
...
var refType = import_prop_types10.default.oneOfType([import_prop_types10.default.func, import_prop_types10.default.object]);
...
function useLazyRef(init, initArg) {
  const ref = React19.useRef(UNINITIALIZED);
...
function getReactElementRef(element) {
  var _a;
  if (parseInt(React25.version, 10) >= 19) {
...
@mui/private-theming/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-7YXKJW5Q.js.map
